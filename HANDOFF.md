...
Deploy the compatible email worker (which accepts records without the new optional superseded field), apply `20260922130000_restore_cancelled_sessions.sql`, then deploy the popup UI. This does not restore existing sessions automatically. Do not replay old cancellation emails after restoration. A Resend testing-sender rejection requires a verified production sender and corresponding `FROM_EMAIL` configuration; rate-limited messages also require controlled retries after that configuration is fixed. `sent_at` means the provider accepted a send, not confirmed inbox delivery.
